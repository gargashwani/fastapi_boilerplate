...
def db_create(database):
    ...
    if connection in ["postgresql", "postgres"]:
        try:
            ...
            click.echo("✗ psycopg2 is required for PostgreSQL. Install with: pip install psycopg2-binary")
            sys.exit(1)
        except psycopg2.OperationalError as e:
            click.echo(f"✗ Error connecting to PostgreSQL: {e}")
            click.echo(f"\nPlease ensure:")
            click.echo(f"  1. PostgreSQL is running")
            click.echo(f"  2. Connection settings in .env are correct")
            sys.exit(1)
        ...
        try:
            ...
            click.echo("✗ pymysql is required for MySQL. Install with: pip install pymysql")
            sys.exit(1)
        except Exception as e:
            click.echo(f"✗ Error: {e}")
            sys.exit(1)
