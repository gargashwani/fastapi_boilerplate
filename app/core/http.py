...
class Http:
    ...
    def retry(self, times: int = 3, retry_on: Optional[List[int]] = None) -> 'Http':
        """
        Set retry attempts.
    ...
    def _build_timeout(self) -> Optional[Timeout]:
    ...
    def _should_retry(self, response: Optional[Response], exception: Optional[Exception] = None) -> bool:
        ...
        if exception:
            return True
        if response and response.status_code in getattr(self, '_retry_on', [500, 502, 503, 504]):
            return True
        return False
    ...
    def _make_request(
        ...
        if hasattr(self, '_record_requests') and self._record_requests:
            self._recorded_requests.append(Request(method, full_url, headers=request_headers))
        ...
        while attempts <= self._retries:
            try:
                ...
                if attempts <= self._retries:
                    logger.warning(f"Request failed, retrying ({attempts}/{self._retries}): {full_url}")
                    continue
                ...
                attempts += 1
                if attempts <= self._retries:
                    logger.warning(f"Request exception, retrying ({attempts}/{self._retries}): {e}")
                    continue
                raise
    ...
    def pool(self, requests: List[Callable]) -> List[HttpResponse]:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            return [HttpResponse(future.result()) for future in concurrent.futures.as_completed(futures)]
    ...
    def assert_sent(self, callback: Optional[Callable] = None) -> bool:
        """
...
_http_instance: Optional[Http] = None
