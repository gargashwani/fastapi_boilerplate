...
class Schedule:
    ...
    def weekly(self) -> 'Schedule':
        ...
        if self._current_job:
            self._current_job['trigger'] = CronTrigger(day_of_week='sun', hour=0, minute=0)
            self._register_job()
        return self
    ...
    def monthly_on(self, day: int, time: str = '0:0') -> 'Schedule':
        ...
        if self._current_job:
            hour, minute = map(int, time.split(':'))
            self._current_job['trigger'] = CronTrigger(day=day, hour=hour, minute=minute)
            self._register_job()
        return self
    ...
    def quarterly(self) -> 'Schedule':
        ...
        if self._current_job:
            self._current_job['trigger'] = CronTrigger(month='*/3', day=1, hour=0, minute=0)
            self._register_job()
        return self
    ...
    def cron(self, minute: str = '*', hour: str = '*', day: str = '*', 
             month: str = '*', day_of_week: str = '*') -> 'Schedule':
        """
    ...
    def _register_job(self):
        ...
        timezone = job_config.get('timezone') or getattr(settings, 'APP_TIMEZONE', 'UTC')
...
_schedule_instance: Optional[Schedule] = None
