...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, socket_id: str, user_id: Optional[int] = None):
    ...
    def disconnect(self, socket_id: str, user_id: Optional[int] = None):
    ...
    async def broadcast_to_channel(self, channel: str, event: str, data: Dict[str, Any], exclude_socket: Optional[str] = None):
                ...
                try:
                    ...
                    if message and message['type'] == 'pmessage':
                        ...
                        try:
                            ...
                            await self.broadcast_to_channel(channel, event, event_data, exclude_socket)
                        except json.JSONDecodeError:
                            logger.error(f"Invalid JSON in broadcast message: {message['data']}")
                except Exception as e:
...
async def authorize_channel(
    ...
    if not channel_name.startswith('private-') and not channel_name.startswith('presence-'):
        raise HTTPException(status_code=403, detail="Channel authorization not required")
